...
-----------------------------------------------------------------
Re-issued with additional fix:
- **Lazy initialization** of Chroma vector stores and Azure embeddings (built on-demand in nodes)
  → avoids import-time failures when environment variables are not set yet.
- Hardened SQL node and improved /chat endpoint retained.

Run:
  python -m uvicorn app_langgraph:app --reload
"""
...
def router_node(state: FlowState) -> FlowState:
    ...
    try:
        ...
        # Fallback: lightweight keyword heuristics if JSON parse fails
...
app = FastAPI(title="Loan Navigator — LangGraph + ChromaDB (lazy init)")
